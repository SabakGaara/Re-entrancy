...
def check_reentrancy_bug(path_conditions_and_vars, stack, global_state,taint_stack):
    ...
    #todo
    # new = source_map.find_source_code(global_state["pc"] - 1)
    # new = new.split('.')[0]
    # for index in range(len(global_state["Ia"])):
    #     var_state_name = str(global_state["Ia"][index])
    #     if var_state_name.startswith("Ia_store"):
    #         try:
    #             var_address = var_state_name.split("-")[1]
    #             var_value = var_state_name.split("-")[2]
    #             #todo
    #             if var_value == new:
    #                 if int(var_address) in global_params.VAR_STATE_GLOBAL:
    #                     if global_params.VAR_STATE_GLOBAL[int(var_address)] == 2:
    #                         log.info("taint_happen in anly")
    #                 else:
    ...
    # 2300 is the outgas used by transfer and send.:w
